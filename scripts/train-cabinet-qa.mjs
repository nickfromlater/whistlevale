...
run(`
 trainCabinet.room='yankee';trainCabinet.draft=collectionDefault('yankee');cabinetUpdateAction();
 assert.equal($('cabinetRun').disabled,false,'the room subway is not the Bergwald preview candidate');
 assert.equal($('cabinetCurrent').textContent,'Currently running · River Avenue Local');assert.equal($('cabinetRestore').hidden,true);
 chooseCollectionTrain('yankee',{id:'tern',livery:0,cars:2});cabinetUpdateAction();assert.equal($('cabinetRestore').hidden,false);
 const roomStorage=localStorage.setItem;localStorage.setItem=()=>{throw Error('private mode');};
 assert.equal(restoreRoomTrain('yankee'),false,'a storage failure is reported while restoring for this visit');localStorage.setItem=roomStorage;
 cabinetUpdateAction();assert.equal($('cabinetCurrent').textContent,'Currently running · River Avenue Local');assert.equal($('cabinetRestore').hidden,true);
 assert.equal(collectionExport().rooms.yankee,undefined);
 restoreRoomTrain('yankee');restoreCollectionSelections();assert.equal(selectedCollection.yankee,undefined,'reload keeps the authored subway after restoration');
 restoreRoomTrain('coast');assert.equal(primary.type,'steam');assert.equal(primary.cars,2);assert.equal(primary.stock,undefined);assert.equal(primary.distance,13);assert.equal(secondary.distance,44);
 const hudsonStock={edge,distance:64,speed:2.2,type:'mountain',stock:'collection:acela',cars:6,collectionChoice:HOUSE_ROOMS.hudson.defaultCollection};roomScenes.set('hudson',{key:'hudson',trains:[hudsonStock]});
 chooseCollectionTrain('hudson',{id:'cinder',livery:0,cars:3});restoreRoomTrain('hudson');
 assert.equal(hudsonStock.collectionChoice.id,'acela');assert.equal(hudsonStock.cars,6);assert.ok(runningCollectionMeshes.has('acela:0'),'restoration prepares the authored collection geometry before switching');assert.equal(hudsonStock.distance,64);
 assert.throws(()=>restoreRoomTrain('qa-landscape'),/no train/);assert.throws(()=>restoreRoomTrain('unknown'),/no train/);
`);
