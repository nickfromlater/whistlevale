...
function element(tag){return {tag,children:[],style:{},attributes:{},hidden:false,textContent:'',append(...nodes){this.children.push(...nodes);},prepend(...nodes){this.children.unshift(...nodes);},setAttribute(k,v){this.attributes[k]=v;},getContext(){return drawing;}};}
...
ui.frame(frame);assert.equal(pin.hidden,false);assert.equal(label.textContent,'Press to reset');
button.onclick();assert.equal(button.attributes['aria-disabled'],'true');assert.equal(label.textContent,'Winding up…');
...
assert.equal(label.textContent,'Press to reset');assert.equal(pin.attributes['aria-disabled'],'false');assert.ok(drawingCalls>100);
...
assert.equal(reducedButton.children[0].textContent,'Codex usage reset!');
for(let i=0;i<110;i++)ui.frame(frame);assert.equal(reducedButton.children[0].textContent,'Press to reset');
