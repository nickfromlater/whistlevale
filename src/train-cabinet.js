...
class CabinetPreview{
 ...
 render(now){
  ...
  const g=this.g,canvas=this.canvas,ratio=Math.min(devicePixelRatio||1,2),width=Math.max(1,Math.round(canvas.clientWidth*ratio)),height=Math.max(1,Math.round(canvas.clientHeight*ratio));if(canvas.width!==width||canvas.height!==height){canvas.width=width;canvas.height=height;}
...
function paintCabinetCards(){
 const list=$('cabinetTrains');list.replaceChildren();const active=collectionChoice(trainCabinet.room);
...
function cabinetUpdateAction(){
 const q=collectionById.get(trainCabinet.draft.id),active=collectionChoice(trainCabinet.room),same=['id','livery','cars'].every(key=>active[key]===trainCabinet.draft[key]);
 $('cabinetRun').disabled=same;$('cabinetRun').innerHTML=(same?'On the railway':'Run '+q.name)+cabinetIcon('arrow');
 $('cabinetCurrent').textContent='Currently running · '+collectionById.get(active.id).name;
}
