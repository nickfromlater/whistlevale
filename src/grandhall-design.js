...
function grandHallWindowSpec(r,x,side){
 ...
 const shape=r.id==='architecture'?[4.3,1.55,h-1.1,false]:r.id==='curiosity'?[2.2,3.2,4,true]:r.id==='conservatory'?[3.8,4.4,3,false]:r.id==='steam'?[3.6,3.2,4.6,false]:r.id==='grand'?[3.5,5.6,4.8,true]:[4,4,3.85,false];
...
function grandHallWall(b,r,side,doors){
 ...
 for(let xx=-half+3.2;xx<half-1.8;xx+=r.id==='steam'?6.4:r.id==='conservatory'?4.8:7.1){
...
function grandHallFloor(b,r){
 const p=r.palette,w=r.width,d=r.depth,stone=['architecture','conservatory','steam'].includes(r.id);
 b.box(0,-.16,0,w,.32,d,p.floor,stone?52:50);
 grandHallBorder(b,0,0,w-1.1,d-1.1,stone?'#e1d5b1':'#c1ac7b');
 if(r.id==='grand'){
  b.box(0,.012,0,5.2,.025,d,'#6b7a61',53);grandHallBorder(b,0,0,4.95,d-.4,'#c6b685',.032);
  b.push(0,.034,1.2,0,0,0,1.0,1,1.30);b.cylinder(0,0,0,3.7,3.7,.018,'#b8aa87',52,48);b.cylinder(0,.012,0,3.35,3.35,.020,'#8c967c',52,48);b.pop();
  for(const s of[-1,1])b.box(s*7.8,.012,-8,8.1,.026,27.0,'#998265',50);
 }else if(r.id==='steam'){
  for(const x of[-8.5,8.5]){b.box(x,.015,-1,5.0,.025,d-5,'#837f6b',52);for(const s of[-1,1])b.box(x+s*2.4,.031,-1,.035,.012,d-5,'#d5c294',41);}
  for(let z=-d/2+1.3;z<d/2;z+=3.5)b.box(0,.012,z,w-.8,.008,.024,'#7e806e',52);
 }else if(r.id==='worlds'){
  for(const [x,z]of[[-8.8,-8.5],[8.8,-8.5],[-8.8,5.5],[8.8,5.5]]){b.push(x,.018,z,0,0,0,1.28,1,1);b.cylinder(0,0,0,5.1,5.1,.018,'#b2b192',53,32);b.pop();}
 }else if(r.id==='conservatory'){
  for(let x=-w/2+1;x<w/2;x+=3)for(let z=-d/2+1;z<d/2;z+=3)b.box(x,.012,z,2.94,.016,2.94,(Math.round(x+z)%2)?'#c9c5a7':'#bebd9c',52);
  for(const x of[-3.3,3.3])b.box(x,.03,0,.045,.015,d,'#8eaa8c',41);
 }else if(r.id==='architecture'){
  for(let x=-w/2+2;x<w/2;x+=3)b.box(x,.012,0,.014,.01,d,'#938f7c',52);
  for(let z=-d/2+2;z<d/2;z+=3)b.box(0,.012,z,w,.01,.014,'#938f7c',52);
  b.box(0,.020,0,4.8,.018,d,'#d0cbb5',52);
 }else if(r.id==='curiosity'){
  b.box(0,.022,0,4.1,.04,d-1,'#5d6755',53);grandHallBorder(b,0,0,3.8,d-1.4,'#b3a172',.045);
  b.cylinder(0,.05,-2,3.3,3.3,.018,'#7f7960',53,8);
 }else{
  for(let z=-d/2+1;z<d/2;z+=1.1)b.box(0,.012,z,w,.01,.018,'#785f43',22);
  for(const x of[-4.5,4.5])b.box(x,.017,0,.06,.018,d-1,'#b5a47b',41);
 }
}
...
function grandHallRoof(b,r){
 ...
 if(r.roof==='vault'){
  ...
  for(const z of[-13,1,14])pendant(b,0,7.7,z);for(const x of[-10.8,10.8])for(const z of[-13,0,13])pendant(b,x,6.6,z);
  ...
  for(let z=-d;z<d-.1;z+=8.5){const end=Math.min(d,z+8.5),peak=z+(end-z)*.72;
   ...
   for(let x=-w+1;x<w;x+=3.6){b.beam([x,h-1.4,z],[x+1.6,h-1.4,peak],.03,p.metal,41,5);}
   ...
   for(const x of[-7.9,7.9])pendant(b,x,5.8,(z+end)/2);
  }
  ...
  for(const x of[-9,9])for(const z of[-8,7])pendant(b,x,5.8,z);
  ...
  for(const side of[-1,1]){
   ...
   for(const z of[-d,0,d])b.beam([side*w,edge,z],[side*opening,base,cz+z/d*opening],.11,p.trim,22,6);
   for(const x of[-w,0,w])b.beam([x,edge,side*d],[x/w*opening,base,cz+side*opening],.11,p.trim,22,6);
  }
  for(const z of[-d+2,-1.8,d-2]){b.beam([-w,edge,z],[w,edge,z],.10,p.trim,22,6);}
  if(octagon){for(const x of[-7.9,7.9])for(const z of[-10.5,-3.7,5.1])pendant(b,x,4.9,z);}
  else for(const x of[-8.5,8.5])for(const z of[-9,5.5])pendant(b,x,5.5,z);
  ...
  for(const x of[-14,-7,7,14])pendant(b,x,5.1,0);
 }
}
function grandHallGarden(b,x,z,size=1){
 b.push(x,0,z,0,0,0,size);b.cylinder(0,.26,0,.70,.83,.52,'#b0af90',52,12);b.cylinder(0,.55,0,.82,.82,.10,'#d0c8a6',52,12);b.cylinder(0,.605,0,.71,.71,.025,'#626c4d',0,12);
 for(let i=0;i<13;i++){const a=i*2.399,r=.32+(i%3)*.10,y=.90+(i%4)*.27,xx=Math.cos(a)*r,zz=Math.sin(a)*r;b.beam([0,.60,0],[xx,y,zz],.018,'#638061',0,5);b.push(xx,y,zz,.35,a);b.sphere(0,.14,0,.16,.40,.05,['#6c8b65','#839a71','#537756'][i%3],0,7,4);b.pop();}b.pop();
}
function grandHallWindowSeat(b,x,z,yaw,w,wood){
 b.push(x,0,z,0,yaw);b.box(0,.45,0,w,.90,.85,wood,22);b.box(0,.95,0,w+.12,.12,.96,'#b3b695',53);
 for(const xx of[-w*.28,0,w*.28]){b.box(xx,.46,.438,w*.27,.58,.025,'#8d9175',22);b.box(xx,.69,.47,.20,.035,.025,'#b9a46e',41);}b.pop();
}
function grandHallRoomDetails(b,r){
 ...
 sign(b,'room-'+r.id,0,5.02,-d+.37,Math.min(8,r.width*.32),.63);
 b.push(0,.052,d-1.25,-Math.PI/2);sign(b,'room-'+r.id,0,0,0,5.2,.38);b.pop();
 if(r.id==='grand'){
  for(const s of[-1,1]){grandHallWindowSeat(b,s*(w-1.0),-1,s<0?Math.PI/2:-Math.PI/2,4.4,p.trim);grandHallGarden(b,s*(w-1.3),-d+1.5,.85);}
  const z=-12.5;b.beam([0,r.height-.35,z],[0,8.05,z],.035,'#bca778',41,6);b.cylinder(0,7.2,z,.82,.82,.23,p.metal,41,36,Math.PI/2);ringZ(b,0,7.2,z+.15,.72,.83,.08,'#bda776',41,36);const q=labels.clock;
  ...
  for(const s of[-1,1])for(const z of[-15,-4,7]){b.box(s*(w-.5),3.4,z,.3,6.8,.7,p.metal,41);b.box(s*(w-.7),.12,z,.9,.24,1.0,'#8b8d78',52);}
  for(const s of[-1,1])grandHallWindowSeat(b,s*7.4,d-1.2,Math.PI,4.2,'#766548');
 }else if(r.id==='worlds'){
  for(const s of[-1,1]){grandHallWindowSeat(b,s*(w-1.05),5.4,s<0?Math.PI/2:-Math.PI/2,5.0,'#9a8965');grandHallGarden(b,s*(w-1.35),-d+1.4,.85);}
 }else if(r.id==='conservatory'){
  for(const x of[-14,-7,7,14])for(const z of[-d+1.0,d-1.0])grandHallGarden(b,x,z,.8);
  for(const s of[-1,1]){grandHallWindowSeat(b,s*(w-.9),0,s<0?Math.PI/2:-Math.PI/2,3.6,'#8b9b7c');for(const z of[-7,7])grandHallGarden(b,s*(w-1.0),z,.76);}
 }else if(r.id==='architecture'){
  for(const s of[-1,1]){
   b.box(s*(w-.7),1.2,-5.4,.75,2.4,4.6,'#b1ab90',52);b.box(s*(w-1.12),1.2,-5.4,.03,1.8,3.8,'#d8d1b6',0);
   grandHallWindowSeat(b,s*(w-1.0),6.1,s<0?Math.PI/2:-Math.PI/2,4.0,'#aba386');
  }
 }else if(r.id==='curiosity'){
  for(const s of[-1,1])grandHallWindowSeat(b,s*7.4,d-1.15,Math.PI,3.5,'#67533f');
 }else{
  // A real shared making bench is room furniture; the numbered displays stay empty.
  const z=-d+1.25;table(b,0,z,6.8,1.7,1.25);for(const x of[-2.2,1.6]){b.box(x,1.34,z,1.6,.025,1.1,'#d3c9a6',0);b.box(x,1.365,z,1.2,.01,.7,'#bdc4a4',0);}
  b.cylinder(2.5,1.45,z,.14,.15,.32,'#8a987b',52,10);for(let i=0;i<5;i++)b.beam([2.45+i*.025,1.42,z],[2.41+i*.06,1.95,z+.04],.012,'#b99859',22,5);
  for(const s of[-1,1]){grandHallWindowSeat(b,s*(w-1.1),-2,s<0?Math.PI/2:-Math.PI/2,5.1,'#9e8763');grandHallGarden(b,s*(w-1.35),-d+1.5,.9);}
 }
}
function grandHallBayShape(b,bay){
 const r=GRAND_HALL_GALLERIES[bay.room],p=r.palette,w=bay.w,d=bay.d,y=bay.y,wood=r.id==='curiosity'?'#755b44':r.id==='workshop'?'#a88d64':'#8c7354',cloth=r.cloth;
 if(bay.furniture==='round'){
  b.cylinder(0,.13,0,w*.38,w*.38,.26,'#959e7e',52,16);b.cylinder(0,(y-.12)/2,0,w*.28,w*.24,y-.24,'#b0b496',52,16);b.cylinder(0,y-.08,0,w/2,w/2,.16,'#c3c6a6',52,24);b.cylinder(0,y+.008,0,w/2-.16,w/2-.16,.016,cloth,53,24);
 }else if(bay.furniture==='honour'||bay.furniture==='landscape'){
  const inset=bay.furniture==='honour'?.20:.28;b.box(0,.11,0,w-.5,.22,d-.5,p.trim,22);b.box(0,y/2,0,w-inset,y-.22,d-inset,p.trim,22);b.box(0,y-.04,0,w,.08,d,'#c0af85',22);b.box(0,y+.006,0,w-.19,.012,d-.19,cloth,53);
 ...
 const face=d/2+.045;b.box(0,y-.22,face,1.24,.34,.07,p.metal,22);sign(b,'bay-'+bay.id,0,y-.21,face+.042,1.0,.25);
}
...
function buildGrandHallArchitecture(){
 ...
 for(const passage of GRAND_HALL_PASSAGES){
  const b=new Builder(),[x0,z0]=passage.p0,[x1,z1]=passage.p1,cx=(x0+x1)/2,cz=(z0+z1)/2,horizontal=x0!==x1,length=Math.hypot(x1-x0,z1-z0),w=passage.width;
  b.push(cx,0,cz,0,horizontal?Math.PI/2:0);b.box(0,-.14,0,w,.28,length+.3,'#a69574',52);b.box(0,.015,0,w-.65,.025,length+.3,'#adb499',52);
  for(const s of[-1,1]){b.box(s*(w/2-.1),.52,0,.2,1.04,length+.25,'#b9bca2',52);b.box(s*(w/2-.1),1.10,0,.35,.16,length+.3,'#d2c7a5',52);b.box(s*(w/2-.1),4.8,0,.14,.14,length+.25,'#748c76',41);}
  for(const z of[-length/2,0,length/2]){for(const s of[-1,1]){b.beam([s*(w/2-.1),1.15,z],[s*(w/2-.1),4.8,z],.055,'#748c76',41,6);b.beam([s*(w/2-.1),4.8,z],[0,5.65,z],.055,'#748c76',41,6);}}
  for(const s of[-1,1])b.quad([s*w/2,4.83,-length/2],[0,5.68,-length/2],[0,5.68,length/2],[s*w/2,4.83,length/2],'#b9c9b1',72);
  b.pop();const mesh=gpu(b.mesh()),record={mesh,model:I,rooms:[passage.from,passage.to]};staticMeshes.push(record);shadowMeshes.push(record);
  createPortal(passage.from,passage.to);createPortal(passage.to,passage.from);
 }
}
