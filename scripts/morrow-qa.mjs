...
context.morrowUpload=data=>{
 ...
 const mesh={count:data.length/12,bounds};meshes.push(mesh);return mesh;
};
...
const report=run(`(()=>{
 ...
 assert.ok(scene.spots.length>=7);assert.equal(walls.length,4);
...
console.log('Morrow House: native geometry, route, portal, stock and motion checks passed.');
