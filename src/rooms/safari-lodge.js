...
function kopjeRoof(b,x,y,z,w,d,rise,variant=0){
 ...
 for(let face=0;face<4;face++){
  ...
  for(let i=0;i<=Math.ceil(columns/5);i++){
   const u=i/Math.ceil(columns/5);b.beam(point(u,0,-.23),point(u,1,-.22),.065,KOPJE.timber,22,6);
  }
 }
...
function kopjeFrame(b,x,z,w,d,h=3.8,rise=2.4){
 const y=SAFARI_LODGE.floor;
 for(const xx of[x-w/2,x,x+w/2])for(const zz of[z-d/2,z+d/2]){
  b.box(xx,y+.09,zz,.44,.19,.44,'#b6a282',4);b.cylinder(xx,y+h/2,zz,.13,.09,h,KOPJE.timber,22,8);
  for(const side of[-1,1])b.beam([xx,y+h-.95,zz],[xx+side*.71,y+h-.10,zz],.06,KOPJE.teak,22,5);
  b.cylinder(xx,y+.28,zz,.144,.144,.11,KOPJE.bronze,41,8);
 }
 for(const zz of[z-d/2,z+d/2])b.box(x,y+h,zz,w+.22,.22,.22,KOPJE.timber,22);
 for(const xx of[x-w/2,x+w/2])b.box(xx,y+h,z,.22,.22,d+.22,KOPJE.timber,22);
 for(const xx of[x-w/2,x,x+w/2]){
  b.beam([xx,y+h,z-d/2],[xx,y+h+rise,z],.11,KOPJE.timber,22,6);
  b.beam([xx,y+h,z+d/2],[xx,y+h+rise,z],.11,KOPJE.timber,22,6);
  b.beam([xx,y+h+.07,z-d/2],[xx,y+h+.07,z+d/2],.065,KOPJE.teak,22,5);
 }
}
...
function kopjeMainHall(b){
 const y=SAFARI_LODGE.floor;
 kopjeFrame(b,24,-.9,12.2,7.6,4.0,3.36);kopjeRoof(b,24,y+4.13,-.9,15.9,10.0,3.45,10);
...
function kopjeWings(b){
 ...
 kopjeFrame(b,13.35,-.55,7.2,6.1,3.25,2.43);kopjeRoof(b,13.35,y+3.4,-.55,9.3,8.4,2.50,35);
 ...
 kopjeFrame(b,35.2,-1.5,6.8,7.2,3.05,2.35);kopjeRoof(b,35.2,y+3.2,-1.5,8.65,9.3,2.42,50);
